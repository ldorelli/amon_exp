...
plt.ylabel("#ReachableNodes")
plt.hist2d(data[0], data[2], bins=50, range=[[0, 0.0002], [0, 200]], norm=LogNorm(), cmap=plt.cm.YlGnBu) 
plt.colorbar()
...
plt.subplot(2, 2, 3)
n, bins, patches = plt.hist(data[2], orientation='horizontal', bins=30, log=True, color=['gray'])
cm = plt.cm.YlGnBu
bin_centers = 0.5 * (bins[:-1] + bins[1:])
col = bin_centers - min(bin_centers)
col /= max(col)
...
# plt.subplot(2, 2, 1)
# plt.scatter(data[0], data[1], s=5, alpha=0.05)
